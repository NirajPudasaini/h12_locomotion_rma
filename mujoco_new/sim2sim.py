...
def get_scale_values(observations_info, actions_info):
    ...
    scales = {
        ...
        'dof_pos_scale': observations_info["joint_pos_rel"]["scale"],
        'dof_vel_scale': observations_info["joint_vel_rel"]["scale"],
        'action_scale': actions_info["joint_effort"]["scale"]
    }
...
def main():
    ...
    parser.add_argument(
        "--config_file", type=str, default="h12.yaml", help="config file name in the config folder"
    )
    ...
    print(f"Scale values - ang_vel: {ang_vel_scale}, dof_pos: {dof_pos_scale}, dof_vel: {dof_vel_scale}, action: {action_scale}")
    ...
    # Initialize target positions to default angles
    target_dof_pos = default_angles.copy()
    
    # Get initial observation and run policy once to get initial action
    qj = d.qpos[7:] - default_angles
    dqj = d.qvel[6:]
    quat = d.qpos[3:7]
    omega = d.qvel[3:6]
    
    qj_scaled = qj * dof_pos_scale
    ...
    obs[6:9] = cmd
    obs[9 : 9 + num_actions] = qj_scaled[joint_ids]
    obs[9 + num_actions : 9 + 2 * num_actions] = dqj_scaled[joint_ids]
    obs[9 + 2 * num_actions : 9 + 3 * num_actions] = action_lab
    
    obs_tensor = torch.from_numpy(obs).unsqueeze(0)
    with torch.no_grad():
        action_lab = policy(obs_tensor).detach().cpu().numpy().squeeze()
    target_dof_pos[joint_ids] = action_lab * action_scale + default_angles[joint_ids]
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                # Policy observation order: base_ang_vel, projected_gravity, velocity_commands, joint_pos_rel, joint_vel_rel, last_action
                obs[0:3] = omega
                obs[3:6] = gravity_orientation
                obs[6:9] = cmd 
                # joint pos
                obs[9 : 9 + num_actions] = qj[joint_ids]
                # joint vel
                obs[9 + num_actions : 9 + 2 * num_actions] = dqj[joint_ids]
                # last actions
                obs[9 + 2 * num_actions : 9 + 3 * num_actions] = action_lab
